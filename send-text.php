...
if (!isset($_SESSION['isAdmin'])  || $isAdmin == 0) {
...
<?php
  ...
  $result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_assoc($result)) { 
		echo <<<TEXT
<div class="checkbox">
 <label><input type="checkbox" id="{$row['firstName']}_check" name="check_employees[]" class="check" value="{$row['firstName']}">{$row['firstName']} {$row['lastName']}</label>
</div>
TEXT;
    }
